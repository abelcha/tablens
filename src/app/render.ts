...
import { buildHeaderLine, buildSeparatorLine, buildBottomSeparatorLine, buildRowLine } from "../utils/text";
...
export function computeTableContentModel(args: {
    ...
    const { headers, visibleRows, rowsOffset, colsOffset, wrapMode, columnOverrides, termW, termH, totalRowCount, selectionMode, cursorCol } = args;
    ...
    const colWidths = computeColumnWidths(dispHeaders, visibleRows.map(r => r.slice(colsOffset)), tableW, adjustedOverrides);
    ...
    const selectedColIdx = (selectionMode === 'column' && cursorCol !== undefined) 
        ? cursorCol - colsOffset 
        : undefined;
    const validSelectedColIdx = (selectedColIdx !== undefined && selectedColIdx >= 0 && selectedColIdx < dispHeaders.length)
        ? selectedColIdx
        : undefined;
