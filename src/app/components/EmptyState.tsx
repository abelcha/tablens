...
export function EmptyState({
    ...
    if (type === "no-results") {
        ...
        sub = query ? `No results for "{white}{bold}${query}{/bold}{/white}"` : "Search returned no results";
        icon = "∅";
        iconColor = "#FF8800";
    ...
    return (
        ...
        >
            <text
                content={parseInlineMarkup(` {bold}${icon}{/bold} `)}
                fg={iconColor}
            />
            {title && (
                <text
                    content={parseInlineMarkup(` {bold}${title}{/bold} `)}
                    fg="#ffffff"
                    marginTop={1}
                />
            )}
